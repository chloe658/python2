...
class Main:
    def __init__(self, parent):
        ...
        self.name_entry = Entry(self.create_order_frame, textvariable=self.name_var).grid(row=self.rowcount1, column=1)

        self.option_label = Label(self.create_order_frame, text="Item: ").grid(row=self.rowcount1+1, column=0)
        self.selected_option = OptionMenu(self.create_order_frame, self.selected_option_var, *self.menu_names).grid(row=self.rowcount1+1, column=0)
        self.add_item_btn = Button(self.create_order_frame, text="Add Item", command=self.add_item).grid(row=self.rowcount1+1, column=1)
        ...
        self.pickup = Radiobutton(self.create_order_frame, text="Pick-Up", value="Pick-Up", variable=self.collection_var).grid(row=self.rowcount1+3, column=0)
        self.delivery = Radiobutton(self.create_order_frame, text="Delivery ($15 Fee)", value="Delivery", variable=self.collection_var).grid(row=self.rowcount1+3, column=1)
        ...
        self.display_order_label = Label(self.display_order, text="Display Orders").grid(row=0, columnspan=2)
        ...
        self.total_price_display.grid(row=2, columnspan=2)
        self.collection_display = Label (self.display_order, text="")
        self.collection_display.grid(row=3, columnspan=2)
        self.item_display = Label(self.display_order, text="Items: ")
        self.item_display.grid(row=4, column=0)
    ...
    def view_orders(self):
        ...
        for i in range(len(self.orders[self.index].item_ordered)):
            Label(text="x" + str(self.orders[self.index].amount[i]) + " " + self.orders[self.index].item_ordered[i]).grid(row=self.rowcount2, column=1, sticky=W)
            self.rowcount2 += 1
        self.total_price_display.configure(text="Total Price: " + str(self.orders[self.index].price))
        self.collection_display.configure(text=f"Collection by {self.collection_var.get()}")
